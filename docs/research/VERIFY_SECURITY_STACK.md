...
| Portal | Status | Notes |
|---|---|---|
| **API Setu** https://apisetu.gov.in/ | **200 OK** | aggregator of central/state govt APIs exists; the /apiDocs path 404'd. **External-API integration since REMOVED (2026-08-26):** consumer invocation is registration-gated, so gov_document no longer calls it — it is treated as discovery/history only |
| **DigiLocker** https://www.digilocker.gov.in/ | **200 OK** | No public developer API documented found in this pass → DigiLocker flows stay behind the app-level QR/sig checks. **External-API integration since REMOVED (2026-08-26);** gov_document keeps DigiLocker only as an official-portal fallback domain, never an API dependency |
...
5. ~~DigiLocker developer-API specifics~~ **RESOLVED → see `INDIA_GOV_VERIFICATION.md` §1:** old developer hosts dead (NXDOMAIN); e-KYC access is partnership-gated (OAuth per UIDAI/DigiLocker partner agreement) — NO-PUBLIC-API for self-serve; **external DigiLocker API integration REMOVED 2026-08-26 (registration-gated, unimplementable)**; QR flow + official-portal fallback retained.
6. ~~Apisetu sub-docs path~~ **RESOLVED → see `INDIA_GOV_VERIFICATION.md` §2:** legacy `apisetu.api.gov.in`/`apisetu.dev.gov.in` NXDOMAIN; live catalog = `directory.apisetu.gov.in` (…; Meilisearch `apidirectory_v2`, 2930 orgs). Read-only discovery is account-free; consumer invocation needs partners.apisetu.gov.in OAuth — **external API-Setu integration REMOVED 2026-08-26 with the same registration gate**.
