...
- **Aadhaar Secure QR:** own decode + RSA-SHA256 signature verification
  ...
  `src/vishwas/assets/uidai_certs/` (no third-party runtime dep).
...
- **Compliance:** Aadhaar Act §29 analysis from the scouting report holds
  under Vishwas's zero-retention design (masked-UID-only output, transient
...
- Certificates are UIDAI-*published public keys* (X.509 PEM despite `.cer`
  ...
  provenance + SHA-256 pins recorded in `src/vishwas/assets/uidai_certs/README.md`.
