...
End-to-end runs through the REAL CLI (`python3 -m vishwas.app cli`) with all
...
Method: sequential heavy runs, `VISHWAS_FFMPEG_THREADS=1`, canonical env
...
```
{"job_dir": "/tmp/vishwas-work/cli-text/job_3251d28c7310", "ts": 1787310416, "reason": "completed", "artifacts_deleted": 2, "failures": 0, "residual_paths": []}
{"job_dir": "/tmp/vishwas-work/cli-smoke_5s.wav/job_4f0d56725376", "ts": 1787310438, ..., "residual_paths": []}
{"job_dir": "/tmp/vishwas-work/cli-clip_av.mp4/job_252497028406", "ts": 1787310473, ..., "residual_paths": []}
{"job_dir": "/tmp/vishwas-work/cli-aadhaar.pdf/job_5136fe4e9502", "ts": 1787310709, ..., "residual_paths": []}
{"job_dir": "/tmp/vishwas-work/cli-stub.exe/job_10f8c692ad65", "ts": 1787310732, ..., "residual_paths": []}
```
Post-run state: `/tmp/vishwas-work/` no longer exists at all (every session
shell removed).
...
- **A. PYTHONPATH fragility** — with bare `PYTHONPATH=src`, torch is
  ...
  `scripts/run_vishwas.sh` (which wires the canonical paths).
- **B. ClamAV guard bug** (`malware_file.py:_clamav`) — `os.access("clamscan",
  ...
  unavailable unless `VISHWAS_CLAMSCAN_BIN` is set to an absolute path.
