...
| `cross_modal` | **Reduced** (consistency heuristics) | HAVIC weight on disk but **architecture class not vendored** (task 1.4 pending) |
...
4. ~~**Download model weights**~~ ✅ **Done 2026-08-20** — aasist/effort×3/havic×2 on disk under `/opt/verisafe/models` (9.26 GB, sha256-verified). `source scripts/provision_weight_env.sh` to export env vars. **Arch-vendor status:** AASIST ✅ (2026-08-21, learned audio tier live) and EFFORT ✅ (2026-08-21, learned video tier live) are vendored and real-weight-verified; **remaining gap: vendor the HAVIC architecture class** (task 1.4) so `is_usable_model()` passes for the cross-modal family; until then that adapter correctly reports `unavailable`.
