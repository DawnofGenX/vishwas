...
**Roadmap:** see `.hermes/plans/2026-08-20_222933-vishwas-full-roadmap.md` — full-scale execution plan (P0 hygiene → P1 learned detectors → P2 delivery/ops → P3 languages → P4 enablement/deploy → P5 re-baseline). This inventory is updated as each phase lands.
...
|---|---|---|---|
| `media-tools` (ffmpeg/ffprobe) | ✅ present | `VISHWAS_FFMPEG_BIN`/`FFPROBE_BIN` point at binaries; threads capped by `VISHWAS_FFMPEG_THREADS` (default 2, thermal-safe) | Video/audio/image probes fail → deepfake targets degrade to `unable_to_verify` with explicit reason |
| `strace` | ✅ present | binary in PATH (or `VISHWAS_CAPE_CMD`) | Dynamic sandbox path limited; static analysis still runs |
...
| `gpg` (binary) | ✅ present + keyring wired (2026-08-20) | `VISHWAS_GPG_TRUSTSTORE` = dir of trusted signer public-key blobs (.asc/.gpg, ownertrust=FULL); optional `VISHWAS_GPG_UNTRUSTED` = dir of known-but-unvouched keys. Zero-network union-keyring model (`src/vishwas/gpg_check.py`): both rings imported into one ephemeral GNUPGHOME before `gpg --verify`; layout-agnostic VALIDSIG/KEY_CONSIDERED parsing; test_15 hermetic (8 tests, real gpg) | `digital_signature` check now runs real verification: trusted→ok, valid-untrusted→degraded+flagged, tampered→failed, no keyring→unavailable |
| `ocr` (tesseract) | ✅ present | in PATH (verified); optionally pin `VISHWAS_TESSERACT_BIN` | scanned-doc OCR works |
...
| `yara` (yara_x) | ✅ **LIVE 2026-08-22** — Finding C closed: curated Mandiant red-team-countermeasures bundle (BSD-2-Clause, pinned commit `3561b717`, 79 files/79 rules) at `rules/yara/` + in-repo MIT starter; all compile under yara_x; E2E-verified firing via CLI (EICAR probe → `yara_x ok hits=1`; benign → `hits=0`) | set `VISHWAS_YARA_RULES=/home/hermes/vishwas/rules/yara` (already added to `deploy/vishwas-secrets.env`, loaded by vishwas-webhook.service); without it the loader falls back to `src/vishwas/assets/yara_rules/` | YARA hit family active (curated bundle); see `rules/yara/PROVENANCE.md` for provenance/update procedure |
...
| `vt` (VirusTotal) | ✅ **LIVE 2026-08-21** — key provisioned, T4.1 probe executed: file path verified end-to-end through project client (EICAR → 65/75 malicious, empty file → 0/75); URL domain path verified (wicar 16-17/91-92, google 0/91). Finding E RESOLVED (2026-08-21 commit e83a69d; regression test added 2026-08-26 `d07e6ca`): exact-url `/urls/{id}` flaky-404s fall back to `/domains/{host}` automatically. Evidence: `docs/research/VT_LIVE_EVIDENCE.md` | free-tier quota (500/day) |
| `llm` (narration) | ❌ no base URL/key | `OPENAI_API_KEY` or `VISHWAS_LLM_BASE_URL`+`API_KEY` (+`LLM_MODEL`) | Optional plain-language narration layer off; structured verdict still sent (template-first) |
| `model-weights` (deepfake/detection models) | ⚠️ **weights provisioned 2026-08-20** (3 of 3 public gates; 9.26 GB on disk) — **arch-vendor status: AASIST ✅ vendored 2026-08-21** (learned audio tier LIVE); **EFFORT ✅ vendored 2026-08-21** (learned video tier LIVE); **HAVIC ✅ vendored 2026-08-21** (learned cross-modal tier LIVE) | inference **adapter registry** in place (`src/vishwas/model_adapters.py`, 7 families keyed by `VISHWAS_{EFFORT,DEMAMBA,FAKEMAMBA,AASIST,SSL_AUDIO,HAVIC,IMAGE_FACE}_WEIGHTS`; deepfake_video/audio + image_facecheck routed through it, test_14 hermetic). On disk under `/opt/vishwas/models/`: **aasist** `best_model.pth` 3,791,651,367 B (sha `febfc126a079…`, torch.load → dict{model_state,optimizer_state,epoch,…}), **effort** 3× checkpoints 1,213,769,519 B each (chameleon/ffpp/genimage, sha `2fc1b970…`/`8d86711f…`/`7c32ceb4…`, OrderedDict 681 tensors), **havic** `best_ft_model.pth` 858,837,738 B (sha `7a0e3ddc…`) + `pt_model.200.pth` 972,770,538 B (sha `a8c44dd5…`). All byte-sizes match manifest ±0.01%; sha256 sidecars written; manifest updated in place. Enable: `source scripts/provision_weight_env.sh` (or `--quiet` for exports only). Skipped-by-design: demamba/fakemamba (no public checkpoint), ssl_audio (requires HF token), image_face (cv2-data path). **AASIST:** architecture class vendored (`model_archs/aasist.py`, WavLM-Large + HtrgGAT); real-weight verify 2026-08-21 → load 15.4 s, inference 13.7 s, `is_usable_model()` True, spoof posterior in range, thermal 49 °C (normal tier) — see `docs/research/ARCH_VENDOR_EVIDENCE_2026-08-21.md`. **EFFORT:** architecture class vendored (`model_archs/effort.py`, CLIP-style ViT-L/14 303M + OrthAlign rank-1 residuals); real-weight verify 2026-08-21 → 100% key coverage (681/681), inference ~1.7 s/image, label order confirmed [real,fake] on known-real photos (posterior 0.29–0.35), thermal 55 °C (normal tier) — same evidence file. **Caveat (havic):** none remaining — architecture class vendored (`model_archs/havic.py` + `_havic/` backend, MIT), wiring live through the adapter seam; real-weight smoke 2026-08-21 → load 8.6 s, inference 5.1 s per clip (NOT SLOW), posterior in range, thermal 48 °C (normal tier) — same evidence file. `pt200` chain entry is a headless pretrain payload that cannot score (documented honestly in the module docstring). |
| `dynamic-sandbox` (Cape/firejail) | ✅ firejail 0.9.72 installed 2026-08-20 (strace remains as fallback) | in PATH; Cape still needs `VISHWAS_CAPE_CMD` if desired | behavioural (sandboxed execution) analysis active |
| `digilocker` / `setu` (gov live APIs) | ❌ **REMOVED 2026-08-26** — external DigiLocker + API-Setu API integrations deleted from `gov_document` (both were registration/partnership-gated, unimplementable without operator credentials) | none (integration removed; env vars ignored) | Unchanged and live: digital-signature/PAdES, QR-native/QR-payload, official-web allowlist lookups, retrieval-only RAG — all local, no registration |
| Fusion checkpoints | ✅ trainer + collector live (2026-08-20) | `python3 -m vishwas.fusion_train --synthetic N --features F --target <name>` or `--dataset corpus.jsonl` (operator-labelled, `{features,label}` rows) → numpy mini-batch-SGD LR head → `$VISHWAS_FUSION_DIR/training/stack_<target>.json` (JSON, same contract as the OOF path); `FusionEngine.load_trained()` wires it; enable at serve time with `VISHWAS_FUSION_USE_TRAINED=1` (default OFF = byte-identical explicit-weight behaviour). Collector: `src/vishwas/dataset_collector.py` (synthesize/load_jsonl/save_jsonl/split/to_dataset_dict, typed DatasetError). test_16 hermetic (7 tests) | Explicit-weight fusion remains the default; provisioned checkpoints add calibrated LR stacking per target once enabled |
| RAG template cache | ◑ seeded (v1, built 2026-08-20) — 10 derived templates, 6 QR schemes, 16 official-content baselines; the api-setu catalog-digest provenance leg was RETIRED 2026-08-26 (sole source was the removed API-Setu catalog); digilocker+nic reachable, incometax/nsdl/epfo unreachable from this box | rebuild via `python3 scripts/build_rag_cache.py` (honors `VISHWAS_RAG_CACHE` + `VISHWAS_RAG_VERSION`) | Template-similarity signal now active for the 10 known doc classes; stale/absent entries degrade confidence silently, never block. Retrieval cache only, never source of truth. |
...
| `gov_document` | **Good** (type ID, field validation, tamper checks, QR-native/QR-payload verification, digital-signature/PAdES, RAG template signal) | tesseract (scans) for OCR-heavy images; external API channels removed 2026-08-26 |
...
| `malicious_file` | Moderate → **Good** (2026-08-22: Finding B fixed — clamscan runs live, EICAR→FOUND E2E; Finding C closed — Mandiant rules bundle live, `Eicar_Test_File` family fires; 5 usable checks fused) | remaining headroom: dynamic sandbox still needs Cape (`VISHWAS_CAPE_CMD`); LLM narration cred-gated |
...
gated exactly as documented: VT key, image_face
weights, LLM narration, HF-token SSL-audio, no-public-checkpoint
demamba/fakemamba. (The DigiLocker/API-Setu external API creds are no longer
gated — that integration was REMOVED 2026-08-26.) Honest gaps found during P5 and recorded (not patched):
...
seam (`VISHWAS_DEVICE`, default cuda-if-available; webhook pinned to cpu so
...
2. **Set a VirusTotal key** (`VISHWAS_VT_API_KEY`) — external reputation for both files and URLs is the highest-value signal-per-effort and needs no local compute.
3. **ClamAV + fresh DB** — local AV for the malware target; keep scans on small/medium files given the thread-cap budget.
4. ~~**Download model weights**~~ ✅ **Done 2026-08-20** — aasist/effort×3/havic×2 on disk under `/opt/vishwas/models` (9.26 GB, sha256-verified). `source scripts/provision_weight_env.sh` to export env vars. **Arch-vendor status:** ALL THREE public-gate families are vendored and real-weight-verified: AASIST ✅ (2026-08-21, learned audio tier live), EFFORT ✅ (2026-08-21, learned video tier live), HAVIC ✅ (2026-08-21, learned cross-modal tier live — load 8.6 s / infer 5.1 s per clip, NOT SLOW).
5. ~~**Retrain fusion**~~ ✅ **Trainer live 2026-08-20** — `fusion_train --synthetic/--dataset` + `dataset_collector` + `VISHWAS_FUSION_USE_TRAINED=1` wiring. Provision a real labelled corpus when available; synthetic path is tested end-to-end.
6. ~~**Wire a GPG keyring**~~ ✅ **Done 2026-08-20** — union-keyring model (`gpg_check.py`), `VISHWAS_GPG_TRUSTSTORE` + `VISHWAS_GPG_UNTRUSTED`, test_15 hermetic. DigitalLocker/SETU credentials still pending (partnership-gated).
