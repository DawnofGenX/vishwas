...
| `yara` (yara_x) | ✅ **LIVE 2026-08-22** — Finding C closed: curated Mandiant red-team-countermeasures bundle (BSD-2-Clause, pinned commit `3561b717`, 79 files/79 rules) at `rules/yara/` + in-repo MIT starter; all compile under yara_x; E2E-verified firing via CLI (EICAR probe → `yara_x ok hits=1`; benign → `hits=0`) | set `VERISAFE_YARA_RULES=/home/hermes/verisafe/rules/yara` (already added to `deploy/verisafe-secrets.env`, loaded by verisafe-webhook.service); without it the loader falls back to `src/verisafe/assets/yara_rules/` | YARA hit family active (curated bundle); see `rules/yara/PROVENANCE.md` for provenance/update procedure |
