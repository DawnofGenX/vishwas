# VeriSafe GPU-Era Rebuild Plan: Fresh Weight Downloads + Better Models + CUDA

> **For Hermes:** Use subagent-driven-development skill to implement this plan task-by-task.

**Goal:** Rebuild VeriSafe's learned-model tier for the new hardware reality — RTX 5090 Laptop (24 GB, sm_120, CUDA 13.0) replaces the thermal-capped i5-8250U CPU assumption. Download all weights fresh, enable the previously-skipped EFFORT tier, move inference to CUDA, and re-baseline performance.

**Architecture:** Three tracks. (A) CUDA enablement: torch 2.13.0+cu130 already sees the GPU in `~/vllm-env`; verisafe currently hardcodes `map_location="cpu"` and runs on CPU — add a device-resolution seam, defaulting to cuda when available with CPU fallback. (B) Weights: fresh downloads of AASIST + HAVIC + RawBMamba via the project fetcher, PLUS EFFORT which was skipped only for thermal reasons (now viable on GPU). (C) Verification: hermetic suite, gate probes, E2E matrix, and a dated PERFORMANCE.md re-baseline (old numbers are CPU-era and obsolete).
...
- **torch**: `~/vllm-env/bin/python` → torch 2.11.0+cu130, `cuda.is_available()=True`, device recognized; `~/docling-python` (the PYTHONPATH tree run_verisafe.sh uses) has torch 2.13.0+cu130
- Repo `/home/hermes/verisafe` clean at `b4a8c0d`; suite collects 336 tests
- `/opt/verisafe/models/` absent — ALL gates down; clamscan not installed
...
- Old plan constraint OBSOLETE: "EFFORT not viable under VeriSafe thermal caps" (fetch_model_weights.sh:68) — GPU removes this
...
**Files:**
- Create: `src/verisafe/device.py`
- Modify: `src/verisafe/model_adapters.py:182-188` (torch.load call)
- Modify: `src/verisafe/model_archs/aasist.py` (score(): move wav + model to resolved device)
- Modify: `src/verisafe/model_archs/havic.py`, `effort.py` (same pattern where tensors/materialized modules exist)
...
def test_resolve_device_prefers_cuda_when_available(monkeypatch):
    ...
    monkeypatch.setenv("VERISAFE_DEVICE", "")
...
def test_resolve_env_override_wins():
    from verisafe.device import resolve_device
    os.environ["VERISAFE_DEVICE"] = "cpu"
    try:
        assert resolve_device() == "cpu"   # explicit override beats autodetect
    finally:
        del os.environ["VERISAFE_DEVICE"]
...
```python
# src/verisafe/device.py
"""Single device-resolution seam for learned-model inference.

Resolution order:
  1. VERISAFE_DEVICE env ("cpu"/"cuda") — explicit operator override
...
## Task 2: Provision /opt/verisafe/models + fetch AASIST & HAVIC

**Objective:** Fresh-download the big checkpoints straight onto the new disk.

**Files:**
- Create: `/opt/verisafe/models/{aasist,havic/best_ft,rawbmamba}/`

**Step 1:** `sudo mkdir -p /opt/verisafe/models/{aasist,havic/best_ft,rawbmamba} && sudo chown -R hermes:hermes /opt/verisafe`
...
```
(correct path prefix `/opt/verisafe/`.)
...
**Files:**
- Create: `/opt/verisafe/models/effort/chameleon/effort_chameleon.pth`
- Modify: `deploy/verisafe-secrets.env` (append VERISAFE_EFFORT_WEIGHTS)

**Step 1:** Read `src/verisafe/model_archs/effort.py` header + `provision_weight_env.sh` to get the exact expected relative path and filename (`effort/chameleon/effort_chameleon.pth` per provision script output).
...
```bash
curl -L --fail -o /opt/verisafe/models/effort/chameleon/effort_chameleon.pth <URL>
```

**Step 4:** Append to `deploy/verisafe-secrets.env`:
```
# EFFORT video/face deepfake tier — enabled 2026-08-24 (GPU box; thermal skip obsolete)
VERISAFE_EFFORT_WEIGHTS=/opt/verisafe/models/effort/chameleon/effort_chameleon.pth
```
...
**Objective:** REPLACE the fakemamba-slot model: download XLSR-Mamba-LA (MIT, better in-the-wild numbers) instead of RawBMamba. New env gate `VERISAFE_XLSRMAMBA_WEIGHTS`; RawBMamba kept as evaluation-grade ONNX fallback only.

**Files:**
- Create: `/opt/verisafe/models/xlsr-mamba/model.safetensors` (from HF `AustinXiao/XLSR-Mamba-LA`, ~1.28 GB incl. XLSR frontend)
- Create: `/opt/verisafe/models/xlsr-mamba/PROVENANCE.md`
- Modify: `deploy/verisafe-secrets.env` (add `VERISAFE_XLSRMAMBA_WEIGHTS=/opt/verisafe/models/xlsr-mamba/model.safetensors`; comment out FAKEMAMBA line with deprecation note)

**Step 1:**
```bash
mkdir -p /opt/verisafe/models/xlsr-mamba
curl -L --fail -o /opt/verisafe/models/xlsr-mamba/model.safetensors \
...
**Step 4:** `systemctl --user restart verisafe-webhook` then `/health` shows the new dep.
...
from verisafe.model_adapters import resolve
from verisafe.device import resolve_device
print('device:', resolve_device())
for g in ['VERISAFE_AASIST_WEIGHTS','VERISAFE_EFFORT_WEIGHTS','VERISAFE_HAVIC_WEIGHTS','VERISAFE_FAKEMAMBA_WEIGHTS']:
    r = resolve(g)
    try:
        obj = r.load(r.path)
...
"
```
IMPORTANT interpreter choice: probe with BOTH `~/vllm-env/bin/python` (known-good CUDA torch) AND system `python3` (which resolves the docling-python tree via PYTHONPATH). If the docling-python torch 2.13.0+cu130 wheel doesn't actually init CUDA on sm_120 (Blackwell needs cu128+; cu130 should be fine but VERIFY — `torch.cuda.get_device_name(0)` inside that interpreter), decide: either standardize run_verisafe.sh on vllm-env's interpreter or accept CPU for the webhook path. Record the decision.
...
**Step 1:** `cd ~/verisafe && PYTHONPATH=src python3 -m pytest tests/ -q` — expect 333+ passed, ≤3 skipped, plus new device-seam tests.

**Step 2:** E2E verdict matrix via `scripts/run_verisafe.sh cli`:
...
- Patch: skill `verisafe-operations` SKILL.md — rewrite "Weight gates", "Thermal protocol" (GPU inference changes it; ffmpeg threads note may persist for decode), "Measured stage costs" sections
...
1. **Blackwell sm_120 compatibility**: cu130 wheels should support sm_120, but the docling-python torch must be verified in-interpreter (Task 6 Step 1 decision point). If broken, options: point run_verisafe.sh PYTHONPATH at vllm-env's site-packages instead, or pip-install matching torch into a dedicated verisafe venv.
...
5. **VRAM co-existence** — see risk 6 (decided): webhook defaults to `VERISAFE_DEVICE=cpu` in secrets env; GPU is opt-in per invocation for heavy runs. `/health` gains a `device` field (Task 6).
6. **VRAM co-existence with local LLM serving — DECIDED (operator confirmed)**: the operator WILL serve local LLMs on the same RTX 5090. Therefore:
   - The device seam (Task 1) MUST support per-run override via `VERISAFE_DEVICE` (already in design) AND the webhook systemd unit must NOT hardcode cuda.
   - Default posture: webhook runs `VERISAFE_DEVICE=cpu` in `deploy/verisafe-secrets.env` so background WhatsApp traffic never competes with the LLM for VRAM; heavy/batch runs are invoked ad-hoc with `VERISAFE_DEVICE=cuda`.
   - Add a Task 6 sub-step: document the "GPU mode" invocation (`VERISAFE_DEVICE=cuda bash scripts/run_verisafe.sh ...`) and add a `/health` field `device` reporting the resolved device so the operator can confirm which mode the webhook is in.
