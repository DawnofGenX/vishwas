...
Task 1 of the GPU-rebuild plan: one seam (verisafe.device.resolve_device)
...
def clean_env(monkeypatch):
    monkeypatch.delenv("VERISAFE_DEVICE", raising=False)
...
def test_resolve_env_override_case_insensitive_cuda(clean_env, monkeypatch):
    from verisafe.device import resolve_device
    monkeypatch.setenv("VERISAFE_DEVICE", "  CUDA ")
