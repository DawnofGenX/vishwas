...
from verisafe.model_archs._wavlm.modules import (
    Fp32GroupNorm,
    Fp32LayerNorm,
    GradMultiply,
    MultiheadAttention,
    ...
    def __init__(
        ...
        def block(
            ...
            if is_layer_norm:
                ...
                # NOTE: the fairseq WavLM-Large checkpoint nests the GroupNorm
                # as 'conv_layers.<i>.2.1.{weight,bias}' — i.e. inside a
                # sub-Sequential whose index-0 slot holds a param-free module.
                # nn.Identity() reproduces that key layout with zero effect on
                # the computation (verified against best_model.pth).
                return nn.Sequential(
                    make_conv(),
                    nn.Dropout(p=dropout),
                    nn.Sequential(nn.Identity(), Fp32GroupNorm(dim, dim, affine=True)),
                    nn.GELU(),
                )
            else:
            ...
            for i, cl in enumerate(conv_layers):
                ...
                self.conv_layers.append(
                    block(
                        ...
                        # NOTE: the provisioned WavLM-Large checkpoint carries a
                        # parametric GroupNorm ('.2.1.{weight,bias}', shape (512,))
                        # in EVERY conv block, not just block 0 as stock fairseq
                        # does. Match the checkpoint (ground truth) -> norm all.
