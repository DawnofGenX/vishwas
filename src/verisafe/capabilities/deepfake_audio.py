...
class DeepfakeAudioCapability:
    ...
    def analyze(self, art: Artifact, ctx: JobContext) -> list[CheckResult]:
        ...
        n_crops = 3
        out.extend(self._multi_crop("fakemamba", "VERISAFE_FAKEMAMBA_WEIGHTS", n_crops, ctx))
        out.extend(self._multi_crop("aasist", "VERISAFE_AASIST_WEIGHTS", n_crops, ctx))
    ...
    def _ssl_detector(self, ctx: JobContext, wav_path: Path) -> list[CheckResult]:
        ...
        env = "VERISAFE_SSL_AUDIO_WEIGHTS"
        m = _load_weights(env)
        if m is None:
...
def _crop_windows(outdir: Path, n: int = 3, win_s: float = 3.0) -> list[Path]:
    ...
    if not _sh.which(os.environ.get("VERISAFE_FFMPEG_BIN", "ffmpeg")) or not src.exists():
        return []
    pi = probe(src)
    total = pi.duration_s
    step = total / max(1, n)
