...
money or confirmation. Pipeline:
  ...
  T1 heavy : gated face-forensics model (EFFORT weights reuse; env override
             VERISAFE_IMAGE_FACE_WEIGHTS allowed to point at a dedicated one)
...
def _resolved_weights_env() -> tuple[str | None, str | None]:
    ...
    VERISAFE_IMAGE_FACE_WEIGHTS first, else fall back to VERISAFE_EFFORT_WEIGHTS."""
    p = os.environ.get("VERISAFE_IMAGE_FACE_WEIGHTS")
    if p:
        return "VERISAFE_IMAGE_FACE_WEIGHTS", p
    p = os.environ.get("VERISAFE_EFFORT_WEIGHTS")
    if p:
        return "VERISAFE_EFFORT_WEIGHTS", p
    return None, None
