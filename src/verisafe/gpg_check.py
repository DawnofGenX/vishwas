...
Operator-managed truststore model:
  - VERISAFE_GPG_TRUSTSTORE (default ~/.verisafe/gpg-truststore) holds public
...
DEFAULT_TRUSTSTORE = Path.home() / ".verisafe" / "gpg-truststore"
_TIMEOUT_S = 60
...
def _gpg_bin() -> str | None:
    return shutil.which(os.environ.get("VERISAFE_GPG_BIN", "gpg") or "gpg")
...
def _blob_primary_fpr(blob) -> str | None:
    ...
    if not available():
        return None
    tmp = Path(tempfile.mkdtemp(prefix="verisafe-gpg-fpr-"))
    try:
        kr = GpgKeyring(tmp)
...
def verify_with_truststore(data_bytes: bytes, sig_bytes_or_path: bytes | Path,
    ...
    ut_dir = (Path(untrusted_dir) if untrusted_dir
              else Path.home() / ".verisafe" / "gpg-known")
    ts_fps = {fp.upper() for fp in load_truststore(ts_dir)}

    base = Path(workdir) if workdir else Path(tempfile.mkdtemp(prefix="verisafe-gpg-run-"))
    kr = GpgKeyring(base)
    try:
        if not kr.usable:
            return GpgVerdict(None, None, None, "gpg binary missing or homedir unusable")
