...
certificates (BEGIN/END CERTIFICATE blocks). `verisafe.qr_verify.aadhaar_secure`
...
4. Sanity-check before committing:
   ...
   for p in sorted(Path("src/verisafe/assets/uidai_certs").glob("*.cer")):
       c = x509.load_pem_x509_certificate(p.read_bytes())
       print(p.name, "->", c.subject.rfc4514_string())
   EOF
   ```

5. Commit. `verisafe.qr_verify` picks up new `.cer` files automatically —
