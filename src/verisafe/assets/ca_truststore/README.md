...
(`verisafe.pades_check.load_trust_store`; default path constant
`_DEFAULT_TRUSTSTORE` in `src/verisafe/capabilities/gov_document.py`,
overridable via `VERISAFE_CA_TRUSTSTORE`).

## Policy

- With no anchor matching a signature's signer, VeriSafe reports the chain as
