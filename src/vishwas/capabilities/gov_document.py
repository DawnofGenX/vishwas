...
def _qr_extra_trust_paths() -> list[str]:
    ...
    as VISHWAS_GPG_TRUSTSTORE). Never points at the production cert dir.
    """
    raw = os.environ.get("VISHWAS_QR_EXTRA_TRUST_PATHS", "")
...
def _extract_text(art: Artifact, ctx: JobContext) -> tuple[str, str]:
    ...
    dl = os.environ.get("VISHWAS_DOCLING")
    if dl and _which(dl):
    ...
    if kind in (MediaKind.PNG, MediaKind.JPEG, MediaKind.WEBP, MediaKind.GIF,
        ...
        exe = _which(os.environ.get("VISHWAS_TESSERACT_BIN", "tesseract"))
        if exe:
            img = art.path
...
def _heic_to_png(art: Artifact, ctx: JobContext) -> Path:
    ...
    ffmpeg = _which(os.environ.get("VISHWAS_FFMPEG_BIN", "ffmpeg"))
    if ffmpeg:
...
_PADES_MOD: Any = None          # cached vishwas.pades_check module (or None)
...
def _get_pades():
    ...
    try:
        ...
        from vishwas import pades_check
...
class GovDocumentCapability:
    ...
    def __init__(self) -> None:
        self.rag_cache_path = Path(os.environ.get(
            "VISHWAS_RAG_CACHE", "/home/hermes/rag-cache")) / "cache-index.json"
        self.rag_version = int(os.environ.get("VISHWAS_RAG_VERSION", "1"))
    ...
    def _digital_signature(self, art: Artifact, ctx: JobContext) -> list[CheckResult]:
        ...
        if b"-----BEGIN PGP" in art.path.read_bytes()[:200_000]:
            ...
            gpg = _which(os.environ.get("VISHWAS_GPG_BIN", "gpg"))
            ...
            if sig_file.exists():
                ts_dir = os.environ.get("VISHWAS_GPG_TRUSTSTORE") or None
                ut_dir = os.environ.get("VISHWAS_GPG_KNOWN_DIR") or None
    ...
    try:
        with sync_playwright() as pw:
            ...
            page = b.new_context(ignore_https_errors=False,
                                 user_agent="Mozilla/5.0 (compatible; VishwasAudit/1.0)").new_page()
...
def _guarded_http_get(url: str) -> str | None:
    ...
    req = urllib.request.Request(f"https://{u.hostname}/",
                                 headers={"User-Agent": "Mozilla/5.0 (compatible; VishwasAudit/1.0)"})
    try:
        import ssl
