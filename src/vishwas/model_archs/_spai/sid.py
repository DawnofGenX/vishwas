...
# IMAGENET_DEFAULT_MEAN / STD inlined from timm.data (re-homed: removes the
# timm runtime dependency for this vendored net).
IMAGENET_DEFAULT_MEAN = (0.485, 0.456, 0.406)
IMAGENET_DEFAULT_STD = (0.229, 0.224, 0.225)

# Re-homed sibling modules (renamed models/utils.py -> net_utils.py). backbones
# (CLIPBackbone/DINOv2Backbone) are NEVER constructor-reachable with the MFM
# ViT backbone used by the SPAI checkpoint, so the import is dropped (annotations
# are lazy under `from __future__ import annotations`).
from . import vision_transformer
from . import filters
from . import net_utils as utils

# Undefined at module level is fine: forward_arbitrary_resolution_batch_with_export
# (the only caller) is the spectral-context-attention export path we do not use
# for scoring. Kept as a `NotImplementedError` stub so the symbol resolves.
def save_image_with_attention_overlay(*args, **kwargs):  # pragma: no cover
    raise NotImplementedError("attention-mask export path not vendored (scoring does not use it)")
