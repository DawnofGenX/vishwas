...
# --------------------------------------------------------
# RE-HOMED imports (mever-team/spai @ 8ff7b3b): the three timm symbols below are
# inlined so this vendored net needs NO timm runtime dependency.
...
class DropPath(nn.Module):
    """Stochastic Depth per sample (inlined from timm.models.layers.DropPath)."""
...
class Attention(nn.Module):
    def __init__(
            ...
            proj_drop=0., window_size=None, attn_head_dim=None, use_flash_attention: bool = False):
        super().__init__()
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.use_intermediate_layers: bool = use_intermediate_layers
        ...
        if self.use_intermediate_layers:
            ...
            cls_head_embed_dim: int = embed_dim * len(self.intermediate_layers)
    ...
    def forward_features(self, x):
        ...
        if self.use_intermediate_layers:
            ...
            # # Additional code not present in original MFM, to perform classification without
            # # fine-tuning the base network.
            #
            # # Discard the old cls token.
            # x = x[:, 1:]
            #
            # # Add a new cls token.
            # actual_cls_tokens = self.actual_cls_token.expand(batch_size, -1, -1)
            # x = torch.cat((actual_cls_tokens, x), dim=1)
            #
            # # Add new learnable positional embeddings.
            # if self.cls_pos_embed is not None:
            #     x = x + self.cls_pos_embed
            # x = self.pos_drop(x)
            #
    ...
    def freeze_base_network(self) -> None:
        modules_to_freeze: list[nn.Module] = [
            self.patch_embed,
            self.cls_token,
            self.pos_embed,
            self.rel_pos_bias,
