...
# Snapshot of this sandbox: `import dns.resolver` at module load pulls in
# dnspython's quic/asyncquery layer which imports the aioquic->service_identity->
# pyopenssl stack; in the serving tree that chain is broken (AttributeError:
# module 'lib' has no attribute 'GEN_EMAIL'). DNS analysis is non-critical and
# already wrapped in try/except inside _analyze_domain, so we import it LAZILY
# there and let a quic-stack failure degrade to the DNS-failed branch (+10 risk)
# instead of crashing the whole scanner at import time. See _phishscanner/
# PROVENANCE.md under "Local rehome".
...
class PhishingDetector:
    ...
    def _analyze_domain(self, domain: str) -> Tuple[int, List[str], Dict]:
        ...
        try:
            # DNS analysis (lazy import so a broken dnspython quic-stack in the
            # serving tree degrades to the DNS-failed branch, never crashes)
